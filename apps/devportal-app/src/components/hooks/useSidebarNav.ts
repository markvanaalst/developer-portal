...
const useSidebarNav = (pageInfo: PageInfo, sidebarNavConfig: SidebarNavigationConfig) => {
  ...
  const [currentLevel, setCurrentLevel] = useState<SidebarNavigationItem[] | null>(null);

  const router = useRouter();
  const root = pageInfo.fileName.endsWith('index.md');

  useEffect(() => {
    const urlSegments: string[] = router.asPath != sidebarNavConfig.path ? router.asPath.replace(sidebarNavConfig.path + '/', '').split('/') : [];
    ...
    let nextItem = findAdjacentRoute(sidebarNavConfig.routes, currentItem.path, true);
    let previousItem = findAdjacentRoute(sidebarNavConfig.routes, currentUrlSegment, false);
    let parentItem = getParentRoute(sidebarNavConfig.routes, currentItem.path);
